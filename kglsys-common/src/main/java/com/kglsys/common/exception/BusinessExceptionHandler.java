...
public class BusinessExceptionHandler extends BaseExceptionHandler {
    ...
    /**
     * 处理自定义的 "资源未找到" 业务异常 (ResourceNotFoundException)。
     * 这个方法只会在Service层或Controller层显式抛出 ResourceNotFoundException 时被调用。
     *
     * @param ex      被抛出的 ResourceNotFoundException 实例
     * @param request 当前的HTTP请求对象
     * @return        一个包含标准错误结构和404状态码的ResponseEntity
     */
